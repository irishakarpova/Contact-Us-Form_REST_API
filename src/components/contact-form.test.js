...
test("Should trigger Required error when name is not filled in on inout blur", async () => {
  ...
  const { getByLabelText, getByTestId } = await render(<ContactForm/>);

  const input = await getByLabelText(labelName);
...
test("Should call onSubmit on submit button click", async () => {
  ...
  const { getByLabelText, getByTestId } = await render(<ContactForm {...props}/>);

  const input = await getByTestId('submit-button');

  await act(async () => {
    await fireEvent.change(getByTestId('name'), { target: { value: 'My name' } });
    await fireEvent.change(getByTestId('email'), { target: { value: 'myemail@local.com' } });
    await fireEvent.change(getByTestId('message'), { target: { value: 'hello' } });
